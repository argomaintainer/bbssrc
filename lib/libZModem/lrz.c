...
static int Crcflg;
static int Firstsec;
/* remove static modifier for gcc4.* */ 
int errors;
...
static int
wcreceive(int argc, const char **argp)
{
	int c;
...
#ifdef PUBDIR
		    || (name[0] == '/' &&
			strncmp(name, PUBDIR, strlen(PUBDIR)))
#endif
		    ) {
			canit(0);
			bibi(-1);
...
tryz(void)
{
	...
	for (n = zmodem_requested ? 15 : 5;
		...
		case ZSINIT:
			 ...
			 */
			Zctlesc |= TESCCTL & Rxhdr[ZF0];
			if (zrdata(Attn, ZATTNLEN, &bytes_in_block) ==
			    GOTCRCW) {
				stohdr(1L);
				zshhdr(ZACK, Txhdr);
				goto again;
		...
		case ZCOMMAND:
			...
			if (zrdata(secbuf, MAX_BLOCK, &bytes_in_block) ==
			    GOTCRCW) {
				zshhdr(ZCOMPL, Txhdr);
				return ZCOMPL;
			}
...
rzfile(struct zm_fileinfo *zi)
{
	...
	for (;;) {
		...
		case ZDATA:
			...
			switch (c =
				zrdata(secbuf + chinseg, MAX_BLOCK,
				       &bytes_in_block))
#else
			switch (c =
				zrdata(secbuf, MAX_BLOCK, &bytes_in_block))
