...
fatal(const char *fmt,...)
{
	va_list args;
	va_start(args, fmt);
	_do_log(SYSLOG_LEVEL_FATAL, fmt, args);
	va_end(args);
	fatal_cleanup();
}
...
error(const char *fmt,...)
{
	va_list args;
	va_start(args, fmt);
	_do_log(SYSLOG_LEVEL_ERROR, fmt, args);
	va_end(args);
}
...
log(const char *fmt,...)
{
	va_list args;
	va_start(args, fmt);
	_do_log(SYSLOG_LEVEL_INFO, fmt, args);
	va_end(args);
}
...
verbose(const char *fmt,...)
{
	va_list args;
	va_start(args, fmt);
	_do_log(SYSLOG_LEVEL_VERBOSE, fmt, args);
	va_end(args);
}
...
debug(const char *fmt,...)
{
	va_list args;
	va_start(args, fmt);
	_do_log(SYSLOG_LEVEL_DEBUG, fmt, args);
	va_end(args);
}
