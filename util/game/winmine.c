...
#define mysrand(seed)	srandom(seed)
#endif
...
checkwin()
{
	...
	// Henry: check the flag to prevent the wrong judgement at last dig
...
winrefresh()
{
	...
	move(0, 22);
	prints
	    ("[1;32m¡î¼üÅÌÉ¨À×¡î[0;1m [[35m°ïÖú: H[37m] [[36mÍË³ö: Q[37m] [[35m´ò¿ª: F[37m] [[36m±êÀ×: S][m\n\r");
	prints
	    ("[v1.00 by zhch.nju 00.3] press \'[1;32mH[m\' to get help, \'[1;32mCtrl+C[m\' to exit.");
	for (y = 1; y <= 16; y++) {
		move(0, y - 1);
		for (x = 1; x <= 30; x++)
			winsh0(x, y);
...
win_loadrec()
{
	...
	for (n = 0; n <= 19; n++) {
		strcpy(topID[n], "ÎÞÃûÊÏ");
		topT[n] = 999;
		strcpy(topFROM[n], "×ÓÐé¿Õ¼ä");
	}
	fp = fopen("reclog/mine2.rec", "r");
	if (fp == NULL) {
		win_saverec();
		return;
	}
