...
*/
	...
	/* Pudding: 针对 PUKEBBS 进行的改动 */	
	bzero((void *) &header, sizeof(header));
	strlcpy(header.filename, name, sizeof(header.filename));
	
	strlcpy(header.owner, userid, sizeof(header.owner));
	strlcpy(header.realowner, header.owner, sizeof(header.realowner));

	str_decode(conv_buf, SUBJECT);
	strncpy(header.title, conv_buf, sizeof(header.title));

	header.flag = 0;
	header.flag |= FILE_OUTPOST;
	...
	int fd, total, i;
	struct stat st;
	struct fileheader *start;	
	for (title_start = header.title; strncmp(title_start, "Re: ", 4) == 0; title_start += 4);
	if (title_start == header.title) goto new_thread;

	header.id = 0;
	

	if ((fd = open(index, O_RDONLY, 0)) == -1) goto new_thread;
	if (fstat(fd, &st) < 0 || st.st_size <= 0) {
		close(fd);
		goto new_thread;
	}
	total = st.st_size / sizeof(struct fileheader);
	if ((start = mmap(NULL, st.st_size, PROT_READ, MAP_SHARED | MAP_FILE, fd, 0)) == NULL) {
		close(fd);
		goto new_thread;
	}
	close(fd);

	for (i = total - 1; i >= 0 && i >= total - THREAD_SEARCH_RANGE; i--) {
		if (strcmp(start[i].title, header.title) == 0 ||
		    strcmp(start[i].title, title_start) == 0) {
			header.id = start[i].id;
			break;
		}
	}	
	munmap(start, st.st_size);
	if (header.id) goto fill_done;
new_thread:
	header.id = now;
fill_done:	
	...
	while (read(fd, &header, size) == size) {
		if (strcmp(file, header.filename) == 0) {
			if (header.owner[0] != '-') {
				...
				strlcpy(header.owner, buf, sizeof(header.owner));
				snprintf(header.title, sizeof(header.title), "<< 本文已被删除 >>");
