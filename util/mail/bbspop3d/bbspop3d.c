...
char *str;
{
	...
	if (strlen(str) + 3 > BUFSIZE)
		str[BUFSIZE - 3] = 0;	//add by gluon for security hole.
...
char *dest;
{
	...
	hp = gethostbyaddr((char *) &rmt_sin->sin_addr,
			   sizeof (struct in_addr), rmt_sin->sin_family);
	if (hp)
...
/*     sprintf(genbuf, "mail/%s/.DIR", LowUserid); */
	sprintf(genbuf, "mail/%c/%s/.DIR", mytoupper(*LowUserid),
		LowUserid);
	...
	for (i = 0; i < totalnum; i++) {
		if (index(fcache[i].owner, '@') == NULL) {
			if (ptr = strchr(fcache[i].owner, ' '))
				*ptr = '\0';
			strcat(fcache[i].owner, ".bbs@"BBSHOST);
		}
		sprintf(genbuf, "mail/%c/%s/%s", mytoupper(*LowUserid),
			LowUserid, fcache[i].filename);
		if (stat(genbuf, &st) == -1)
			st.st_size = 0;
		postlen[i] =
...
main(int argc, char **argv)
{
	...
	setsockopt(msock, SOL_SOCKET, SO_REUSEADDR, (char *) &on,
		   sizeof (on));
	...
	while (1) {
		...
		case 0:	/* child process */
			...
			sprintf(genbuf,
				"+OK InternetBBS Pop server at %s starting.",
				hostname);
			outs(genbuf);
...
Retr()
{
	...
	if (num <= 0 || totalnum < num) {
		...
		outs(genbuf);
		return -1;
	} else if (fcache[num - 1].flag & FILE_DIGEST) {
	...
	outs(genbuf);
	sprintf(genbuf, "To: %s.bbs@"BBSHOST, currentuser.userid);
	outs(genbuf);
	sprintf(genbuf, "Subject: %s", fcache[num].title);
	outs(genbuf);
	outs("");
	sprintf(genbuf, "mail/%c/%s/%s", mytoupper(*LowUserid), LowUserid,
		fcache[num].filename);
...
Rset()
{
	...
	for (i = 0; i < totalnum; i++) {
		fcache[i].flag &= ~FILE_DIGEST;
	}
	markdel = 0;
	sprintf(genbuf, "+OK Maildrop has %d messages (%d octets)",
		totalnum, totalbyte);
	outs(genbuf);
	return 0;
}
...
List()
{
	...
	if (*cmd == 0) {
		...
		outs(genbuf);
		for (i = 0; i < totalnum; i++) {
			if (!(fcache[i].flag & FILE_DIGEST)) {
				sprintf(genbuf, "%d %d", i + 1,
					postlen[i]);
				outs(genbuf);
			}
		}
		...
		i = atoi(cmd);
		if (i <= 0 || totalnum < i) {
			sprintf(genbuf, "-ERR Message %d does not exist.",
				i);
			outs(genbuf);
			return -1;
		} else if (fcache[i - 1].flag & FILE_DIGEST) {
			sprintf(genbuf,
				"-ERR Message %d has been deleted.", i);
			outs(genbuf);
			return -1;
		}
...
Dele()
{
	...
	if (num <= 0 || totalnum < num) {
		...
		sprintf(genbuf,
			"-ERR Message %d has already been deleted.", num);
		outs(genbuf);
		return -1;
	}
...
do_delete()
{
	...
	sprintf(fpath, "mail/%c/%s/.DIR", mytoupper(*LowUserid),
		LowUserid);
	sprintf(fnew, "mail/%c/%s/pop3.DIR", mytoupper(*LowUserid),
		LowUserid);
...
/*modified by KCN for not delete marked mail. 98.11    	*/
		if (i >= totalnum || !(fcache[i].flag & FILE_DIGEST) ||
		    (fcache[i].flag & FILE_MARKED)) {
			write(fdw, &currentmail, sizeof (currentmail));
			count++;
		} else {
			sprintf(genbuf, "mail/%c/%s/%s",
				toupper(*LowUserid), LowUserid,
